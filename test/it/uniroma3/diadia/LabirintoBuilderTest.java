...
public class LabirintoBuilderTest {
	...
	public void testPiuDiQuattroAdiacenti() {
		Labirinto maze = labirintoBuilder
				...
				.getLabirinto();
		assertEquals("stanza 1",maze.getStanzaIniziale().getStanzaAdiacente(Direzione.NORD).getNome());
		assertEquals(4, maze.getStanzaIniziale().getMapStanzeAdiacenti().size());
	...
	public void testAggiuntaAttrezzoAStanze_AppenaAggiunteMultiple() {
		...
		Attrezzo attrezzo = new Attrezzo(nomeAttrezzo,peso);
		// NUOVA RIGA CORRETTA:
		List<Attrezzo> attrezzi = new java.util.ArrayList<>(labirintoBuilder.getListaStanze().get(nomeStanza).getAttrezzi());
